...
def create_bpe(bpe_name, vocabulary = set()):
    ...
    args = parser.parse_args( ("-c " + bpe_name).split())

    if len(vocabulary) != 0:
        print("######## BPE: Enabled Vocabulary Surpressing!")
    ...
    bpe = BPE(args.codes, args.merges, args.separator, vocab = vocabulary, glossaries = args.glossaries)
    return bpe
...
if __name__ == "__main__":
    ...
    import os
    bpe_path = "/home/guojy/harold/main/subword-nmt/codes_30K.bpe"
    bpe = create_bpe(bpe_path)

    for i in os.listdir(train_file_path):
        print(i)
        output = codecs.open("/home/guojy/harold/main/elmo/data/one_billion_bpe/train/" + i +".bpe", 'w',encoding='utf-8')
        input = codecs.open(train_file_path + i, encoding='utf-8')
        for line in input:
            output.write(bpe.process_line(line))

    for i in os.listdir(valid_file_path):
        print(i)
        output = codecs.open("/home/guojy/harold/main/elmo/data/one_billion_bpe/dev/" + i +".bpe", 'w', encoding='utf-8')
        input = codecs.open(valid_file_path + i, encoding='utf-8')
        for line in input:
            output.write(bpe.process_line(line))
