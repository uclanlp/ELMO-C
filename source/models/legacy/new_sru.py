...
if torch.cuda.device_count() > 0:
    #try:
    from .sru_cuda_code import SRU_Compute_GPU
    #except:
    #    from sru_cuda_code import SRU_Compute_GPU
    ...
    def forward(self, input, c0=None, mask_pad=None, return_proj=False):
        ...
        if self.n_proj > 0:
            ...
            if self.clip_value != -1:
                x_projected = torch.clamp(x_projected,
                  -self.clip_value,
                  self.clip_value)
...
class SRU(nn.Module):
    ...
    def forward(self, input, c0=None, mask_pad=None, return_hidden=True):
        ...
        for i, rnn in enumerate(self.rnn_lst):
            ...
            '''reshape_h = h.view(-1).detach().cpu().numpy()
            counter = 0
            for j in reshape_h:
                if j > 3 or j < -3:
                    counter += 1
            print(reshape_h.shape)
            print("{}the layer: {} percent of the values are bad.".format(i, counter / reshape_h.shape[0]))
            '''
    ...
    def get_all_layer_output(self, input, c0=None, mask_pad=None, return_hidden=True):
        """
        Feeds `input` forward through `num_layers` `SRUCell`s, where `num_layers`
        is a parameter on the constructor of this class.
        """
        ...
        lstc = []
        for i, rnn in enumerate(self.rnn_lst):  
            h, c, proj = rnn(prevx, c0[i], mask_pad=mask_pad, return_proj = True)

            '''reshape_h = proj.view(-1).detach().cpu().numpy()
            counter = 0
            for j in reshape_h:
                if j > 3 or j < -3:
                    counter += 1
            ...
            reshape_h = prevx.view(-1).detach().cpu().numpy()
            counter = 0
            for j in reshape_h:
                if j > 3 or j < -3:
                    counter += 1
            print(reshape_h.shape)
            print("{}the layer (after layernorm): {} percent of the values are bad.".format(i, counter / reshape_h.shape[0]))'''
            prevx = self.ln_lst[i](h) if self.use_layer_norm else h
            if self.n_proj:
                proj_list.append(proj)
            else:
                proj_list.append(prevx)
            lstc.append(c)

        if return_hidden:
            return torch.stack(proj_list), torch.stack(lstc) # Here we just return the all layers output
        else:
            return prevx
