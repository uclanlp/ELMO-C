...
class SRU(nn.Module):
    ...
    def forward(self, input, c0=None, return_hidden=True):
        ...
        if c0 is None:
            ...
            assert(c0.size(1) == input.size(1)) # very necessary check
            assert(c0.size(2) == self.n_out)
    ...
    def get_all_layer_output(self, input, c0):

        # ONLY USE IF INPUT LENGTH IS ONE
        assert input.dim() == 3 # (len, batch, n_in)
        dir_ = 2 if self.bidirectional else 1
        if c0 is None:
            zeros = Variable(input.data.new(
        ...
        for i, rnn in enumerate(self.rnn_lst):
            h, c = rnn(prevx, c0[i])
            prevx = self.ln_lst[i](h) if self.use_layer_norm else h
            out_list.append(prevx)
            lstc.append(c)

        return torch.stack(out_list, dim = 0), torch.stack(lstc)

class SRUWithProjection(nn.Module):
    def __init__(self, input_size, projection_dim, hidden_size, num_layers=2, dropout=0,
                 rnn_dropout=0, bidirectional=False, use_tanh=1, use_relu=0,
                 use_selu=0, weight_norm=False, layer_norm=False,
                 highway_bias=0, rescale=True):
        super(SRUWithProjection, self).__init__()
        ...
        for i in range(num_layers):
            l = SRUCell(
                n_in = self.n_in if i==0 else self.projection_dim,
                n_out = self.n_out,
                dropout = dropout if i+1 != num_layers else 0,
                rnn_dropout = rnn_dropout,
                bidirectional = bidirectional,
            ...
            self.projection_lst.append(nn.Linear(self.n_out, self.projection_dim, bias = False))

            if layer_norm:
                self.ln_lst.append(LayerNorm(self.projection_dim))
    ...
    def get_all_layer_output(self, input, c0):
        # We can only call this when the the input's length is 1
        ...
        for i, rnn in enumerate(self.rnn_lst):
            ...
            prevx = self.ln_lst[i](h) if self.use_layer_norm else h
            h = torch.clamp(h, -3, 3)
            lstc.append(c) # because h contains the whole outputs while c contains only the last time step
            out_list.append(prevx)
        return torch.stack(out_list, dim = 0), torch.stack(lstc, dim = 0)
