...
from .log_uniform import util

class SampledSoftmax(nn.Module):
    def __init__(self, ntokens, nsampled, nhid, tied_weight, n_proj, sparse = True, speed_up =False):
        ...
        if not sparse:
            ...
            if speed_up:
                softmax_b = torch.nn.Embedding(ntokens, 1, sparse=True)
                softmax_w = torch.nn.Embedding(ntokens, nhid, sparse=True)
                self.softmax_w = softmax_w.weight.data
                self.softmax_b = softmax_b.weight.data
            else:
    ...
    def sampled(self, inputs, labels, sample_values, remove_accidental_match=False):
        ...
        if self.sparse:
            if self.speed_up:
                ...
                true_weights = self.softmax_w[labels.cpu()]
                true_bias = self.softmax_b[labels.cpu()].squeeze(1)
            else:
                sample_weights = self.softmax_w(sample_ids)
                sample_bias = self.softmax_b(sample_ids).squeeze(1)
            ...
            if self.speed_up:
                sample_weights = sample_weights.cuda(device_id)
                sample_bias = sample_bias.cuda(device_id)
                true_weights = true_weights.cuda(device_id)
                true_bias = true_bias.cuda(device_id)
        else:
